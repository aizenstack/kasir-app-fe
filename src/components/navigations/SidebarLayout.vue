...
<template>
  <aside
    class="hidden md:flex flex-col w-64 bg-gradient-to-b from-white via-blue-50/30 to-white border-r border-blue-200/50 shadow-xl backdrop-blur-sm px-5 py-6 relative overflow-hidden"
  >
    <div class="absolute top-0 right-0 w-32 h-32 bg-blue-400/20 rounded-full blur-3xl -z-0 animate-pulse"></div>
    <div class="absolute bottom-0 left-0 w-40 h-40 bg-indigo-400/20 rounded-full blur-3xl -z-0 animate-pulse" style="animation-delay: 1s;"></div>
    ...
    <nav class="flex flex-col gap-2 relative z-10">
      ...
      >
        <div v-if="$route.path === item.to" class="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000"></div>
        
        <div
          class="w-10 h-10 flex items-center justify-center rounded-xl transition-all duration-300 relative z-10"
          :class="{
